...
class VoucherContentHelper {
  ...
  /// Generates a barcode as an image that can span the full paper width
  static Future<String> generateBarcodeImage(
    String data, {
    int width = 383, // Default to ContentGeneratorParams.textBlockWidth as int
    int height = 80,
    Barcode? barcodeType,
  }) async {
    try {
      // Use Code128 as default - good for alphanumeric data
      final barcode = barcodeType ?? Barcode.code128();

      // Create a bitmap image using the image package
      final barcodeImage = Image(width: width, height: height);
      fill(barcodeImage, color: ColorRgb8(255, 255, 255)); // White background
      ...
      // Draw barcode elements on image
      for (final element in elements) {
        if (element is BarcodeBar && element.black) {
          // Draw black bars
          final left = element.left.toInt();
          final top = element.top.toInt();
          final right = (element.left + element.width).toInt();
          final bottom = (element.top + element.height).toInt();

          for (var y = top; y < bottom && y < height; y++) {
            for (var x = left; x < right && x < width; x++) {
              barcodeImage.setPixel(x, y, ColorRgb8(0, 0, 0));
            }
          }
        }
      }

      // Convert to monochrome and compress (same as convertImage method)
      final gray = grayscale(barcodeImage);
      final imageWidth = gray.width;
      final imageHeight = gray.height;
      final bytesPerRow = (imageWidth + 7) ~/ 8;
      ...
      final imageCommand =
          '^GFA,$totalBytes,$totalBytes,$bytesPerRow,:Z64:$z64';

      return imageCommand;
    } catch (e) {
      log('Error generating barcode image: $e');
      // Fallback to empty image if barcode generation fails
      return '^GFA,0,0,0,:Z64:';
    }
  }
}
