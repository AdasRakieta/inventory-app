...
import 'package:ok_mobile_datawedge_scanner/models/profile_model.dart';
import 'package:ok_mobile_datawedge_scanner/models/scanprofile.dart';
import 'package:ok_mobile_datawedge_scanner/models/scanresult.dart';
import 'package:ok_mobile_datawedge_scanner/scancommands.dart';
import 'package:ok_mobile_datawedge_scanner/scanwedge_channel.dart';

export 'package:ok_mobile_datawedge_scanner/models/aimtype.dart';
export 'package:ok_mobile_datawedge_scanner/models/barcode_plugin.dart';
export 'package:ok_mobile_datawedge_scanner/models/barcodetype_enum.dart';
export 'package:ok_mobile_datawedge_scanner/models/plugin_names.dart';
export 'package:ok_mobile_datawedge_scanner/models/profile_model.dart';
export 'package:ok_mobile_datawedge_scanner/models/scanprofile.dart';
export 'package:ok_mobile_datawedge_scanner/models/scanresult.dart';

class Scanwedge {
  final ScanwedgeChannel _scanwedgeChannel;
  static Scanwedge? _instance;
  static Future<Scanwedge>? _instanceFuture;

  Scanwedge._(this._scanwedgeChannel);

  /// Returns the singleton instance of Scanwedge, initializing if needed.
  static Future<Scanwedge> getInstance() {
    if (_instance != null) {
    ...
    _instanceFuture = ScanwedgeChannel.initialize().then((channel) {
      _instance = Scanwedge._(channel);
      return _instance!;
    });
    return _instanceFuture!;
  }

  /// For backward compatibility, but will always return a new instance (not recommended)
  @Deprecated(
    'Use Scanwedge.getInstance() instead to avoid multiple initializations.',
  )
  ...
  Future<bool> createProfile(ScanProfile profile) =>
      switch (_scanwedgeChannel.supportedDevice) {
  ...
  Future<bool> createScanProfile(ProfileModel profile) =>
      _scanwedgeChannel.createProfile(profile: profile);
  ...
  Future<void> suspendScanner() => _scanwedgeChannel.sendCommand(
    command: ScanCommands.scannerInputPlugin,
    parameter: 'SUSPEND_PLUGIN',
  );
  ...
  Future<void> resumeScanner() => _scanwedgeChannel.sendCommand(
    command: ScanCommands.scannerInputPlugin,
    parameter: 'RESUME_PLUGIN',
  );
  ...
  @Deprecated(
    'This is for backwards compatibility and only support Zebra devices, this will be removed later',
  )
  ...
  }) => _scanwedgeChannel.sendCommandBundle(
    command: command,
    parameter: parameter,
