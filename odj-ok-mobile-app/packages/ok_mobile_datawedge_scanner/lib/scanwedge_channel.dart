...
class ScanwedgeChannel {
  ...
  @Deprecated(
    'This is for backwards compatibility, use isDeviceSupported instead',
  )
  ...
  static Future<ScanwedgeChannel> initialize() async {
    try {
      if (!kIsWeb && Platform.isAndroid) {
        final supportedResponse = await _methodChannel.invokeMethod<String>(
          'initializeDataWedge',
        );
  ...
  static SupportedDevice _fetchSupportedDevice(String apiVersion) =>
      switch (apiVersion) {
  ...
  Future<bool> toggleScanning() async => isDeviceSupported
      ? await _methodChannel.invokeMethod<bool>('toggleScanning') ?? false
      : false;
  Future<bool> enableScanner() async => isDeviceSupported
      ? await _methodChannel.invokeMethod<bool>('enableScanner') ?? false
      : false;
  Future<bool> disableScanner() async => isDeviceSupported
      ? await _methodChannel.invokeMethod<bool>('disableScanner') ?? false
      : false;
  ...
  Future<bool> sendCommandBundle({
    ...
    final invoked = await _methodChannel.invokeMethod<bool>(
      'sendCommandBundle',
      {'command': command, 'parameter': parameter, 'sendResult': sendResult},
    );
