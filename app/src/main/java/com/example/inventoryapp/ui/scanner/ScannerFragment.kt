...
class ScannerFragment : Fragment() {
    ...
    private var scanMode: String = "product"
        ...
        viewModel.scanResult.observe(viewLifecycleOwner) { state ->
            when (state) {
                is ScannerViewModel.ScanState.Success -> {
                    ...
                    when (scanMode) {
                        "qr_import" -> {
                            // Return scanned data to ExportImportFragment
                            setFragmentResult("qr_scan_result", bundleOf("scan_data" to state.code))
                            requireActivity().onBackPressedDispatcher.onBackPressed()
                        }
                        else -> {
                            // Default product scan mode
                            if (productId != -1L) {
